...
"""
CSV → SQLite 변환 스크립트
step7_evms_04.csv → project_db_v3.sqlite
2026-02-19 업데이트: 실행률 반영
"""
import csv
import sqlite3
import os
...
def main():
    ...
    # 오늘 날짜 기준 진행 중인 작업 확인
    today = '2026-02-19'
    active = cursor.execute(
        f'SELECT COUNT(*) FROM evms WHERE "WHEN1_시작일" <= "{today}" AND "WHEN2종료일" >= "{today}"'
    ).fetchone()[0]
    print(f'Active tasks today ({today}): {active}')

    # 본관동 B1 작업 확인
    b1_active = cursor.execute(
        f'SELECT COUNT(*) FROM evms WHERE "WHERE2_동" = "01_본관동" AND "WHERE3_층" = "02_B1" AND "WHEN1_시작일" <= "{today}" AND "WHEN2종료일" >= "{today}"'
    ).fetchone()[0]
    print(f'Active B1 tasks today: {b1_active}')

    # EV 계산
    ev = cursor.execute(
        'SELECT SUM("R10_합계_금액" * COALESCE("WHEN4_실행률(%)", 0)) FROM evms'
    ).fetchone()[0]
    print(f'EV (from progress): {ev:,.0f}')
    print(f'EV / BAC = {ev/total:.2%}' if total else '')
