...
> **마지막 업데이트**: 2026-02-17 21:56 KST  
> **Git**: https://github.com/bignine99/ai_pmis.git (main 브랜치, 커밋 `05faad4`)  
> **파일 위치**: `c:\Users\cho\Desktop\Temp\05 Code\260216_evms_dashborad\`  
> **캐시 버전**: 모든 CSS/JS 파일은 `?v=13`
...
- ⚠️ **API 호출 테스트 필요**: `responseMimeType` 제거 후 Gemini 응답이 정상적으로 JSON을 반환하는지 확인 필요
- ⚠️ **구어체 사전 확장**: 사용자가 현장 비속어 목록을 정리해 주기로 함 → CSV 사전 파일로 구현 예정
...
| 공정 관리 | `#schedule` | ✅ 완료 | Gantt 차트 (전체/분기/주간/마일스톤) |
...
| **AI 분석** | `#ai-analysis` | ⚠️ 진행중 | 자연어 질의, 차트 자동 생성, 프리셋 |
| CUBE View | `#cube-view` | ✅ 완료 | 3D 다차원 분석 뷰 |
...
### 🔴 높음
1. **Gemini API 연동 테스트** — `responseMimeType` 제거 후 실제 API 응답 확인
   - F12 Console에서 `[AI] Trying model:`, `[AI] Parsed SQL:` 로그 확인
   - 에러 시 채팅에 상세 메시지 표시됨
   
2. **AI 응답 결과 표시** — SQL 실행 결과가 캔버스(Summary/Grid/Chart 탭)에 정상 렌더링되는지 확인

### 🟡 중간
3. **건설 구어체 사전 확장** — 사용자가 현장 비속어 목록 제공 예정
   - `data/slang_dictionary.csv` 파일로 외부화 권장
   - 현재는 `ai_engine.js`의 `SYNONYMS` 배열에 하드코딩됨

4. **AI 분석 UX 개선**
   - AI 응답 텍스트가 너무 길면 채팅에서 120자로 잘림 → 캔버스에서 전체 확인
   - 채팅 메시지 안에 에러 상세가 나오는지 확인

### 🟢 낮음  
5. **기타 페이지 미세 조정** — 간트 마일스톤 정확도, 비용 차트 디테일 등

---

## 11. 알려진 기술적 주의사항

1. **캐시**: 파일 수정 후 반드시 `index.html`의 `?v=` 번호를 올려야 함 (현재 `v=13`)
